...
import AnimatedSportsCard from '@/src/components/AnimatedSportsCard';
import { getAllAnnouncements } from '@/src/api/services/firestore/announcementsService';
import { getLocalFavoriteDelegation } from '@/src/api/services/storage/favoriteDelegationService';
import { getLocalSupportedSports } from '@/src/api/services/storage/supportedSportsService';
...
const ProfileScreen: React.FC = () => {
    ...
    const fetchPreferences = async () => {
        getLocalFavoriteDelegation().then((delegation) => {
            setSelectedTeam(delegation);
        });
        getLocalSupportedSports().then((sports) => {
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
            <SafeAreaView style={[styles.container,{paddingBottom: insets.bottom}]}>
                ...
                <View style={styles.additional_info_container}>
                    <Text style={{ textAlign: 'center', fontSize: 25, fontWeight:'bold', paddingHorizontal: 10 }}>
                        {"Infos pratiques"}
                    </Text>
                    ...
                    <View style={{flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-around', marginTop: 20}}>
                        <TouchableOpacity 
                            ...
                            onPress={() => router.push('/other/rules')}
                        >
                            <Image 
                                source={require('@/assets/images/icons/rules.png')} 
                                style={styles.info_card_icon}
                            />
                            <Text style={styles.info_card_title} numberOfLines={1}>Règles</Text>
                        </TouchableOpacity>

                        <TouchableOpacity 
                            style={styles.info_card}
                            onPress={() => router.push('/other/lunch')}
                        >
                            <Image 
                                source={require('@/assets/images/icons/lunch.png')} 
                                style={styles.info_card_icon}
                            />
                            <Text style={styles.info_card_title} numberOfLines={1}>Repas</Text>
                        </TouchableOpacity>
                        
                        <TouchableOpacity 
                            style={styles.info_card}
                            onPress={() => router.push('/other/contact')}
                        >
                            <Image 
                                source={require('@/assets/images/icons/contact.png')} 
                                style={styles.info_card_icon}
                            />
                            <Text style={styles.info_card_title} numberOfLines={1}>Contact / Réseaux</Text>
                        </TouchableOpacity>
                        
                        <TouchableOpacity 
                            style={styles.info_card}
                            onPress={() => router.push('/other/about')}
                        >
                            <Image 
                                source={require('@/assets/images/icons/about.png')} 
                                style={styles.info_card_icon}
                            />
                            <Text style={styles.info_card_title} numberOfLines={1}>À propos</Text>
                        </TouchableOpacity>
                    </View>
                </View>
                ...
                {initializing ?
                    ...
                    :
                    (currentUser ? 
                        <TouchableOpacity onPress={signOutButtonPressed} style={styles.login_button_container}>
                            <Text style={{ color: 'blue' }}>Se déconnecter</Text>
                        </TouchableOpacity>
                        :
                        <TouchableOpacity onPress={signInButtonPressed} style={styles.login_button_container}>
                        ...
                        <View style={styles.modal_list_container}>
                            ...
                            />
                                <TouchableOpacity onPress={() => handleSchoolSelected(null)} style={{ margin: 10, padding: 10, backgroundColor: '#4287f5', borderRadius: 20 }}>
                                    <Text style={{ color: 'white', fontWeight: 'bold' }}>{loadingUser ? 'Chargement...' : 'Réinitialiser'}</Text>
                                </TouchableOpacity>
                        </View>
                    </View>
                        ...
                        <View style={styles.modal_list_container}>
                            ...
                            />
                                <TouchableOpacity onPress={handleSportsChanges} style={{ margin: 10, padding: 10, backgroundColor: '#4287f5', borderRadius: 20 }}>
                                    <Text style={{ color: 'white', fontWeight: 'bold' }}>{loadingUser ? 'Chargement...' : 'Terminer'}</Text>
                                </TouchableOpacity>
                        </View>
                    </View>
                </TouchableWithoutFeedback>
